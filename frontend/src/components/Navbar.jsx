  ...
  return (
    <nav className="bg-gray-900 text-white px-6 py-4 flex justify-between items-center w-full fixed top-0 left-0 z-50">
      ...
      <div className="relative flex items-center">
        <div className="flex items-center cursor-pointer space-x-2" onClick={() => setShowDropdown(!showDropdown)}>
          <img src="/userlogoprof.png" alt="Profile" className="w-10 h-10 rounded-full border-2 border-gray-300 hover:opacity-80" />
            ...
            {loading ? (
              ...
              <>
                <div className="flex flex-col items-center">
                  <img src="/userlogoprof.png" alt="Profile" className="w-16 h-16 rounded-full border-2 border-gray-400" />
                  <h3 className="font-bold text-lg mt-2">{user?.name || "Unknown User"}</h3>
                  <p className="text-sm font-semibold text-blue-600">Role: {user?.role || "Unknown"}</p>
                </div>
                <div className="mt-4 flex flex-col">
                  <button className="py-2 px-4 bg-gray-200 rounded-md hover:bg-gray-300">Profile</button>
                  <button onClick={handleLogout} className="py-2 px-4 bg-red-500 text-white rounded-md mt-2 hover:bg-red-600">Sign Out</button>
                </div>
              </>
            )}
          </div>
